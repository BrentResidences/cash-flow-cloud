...
def job_options_list(jobs_df, selected_company=None, selected_bucket=None):
    ...
    if selected_company:
        work = work[work["Company Name"].astype(str) == selected_company]
    if selected_bucket:
        work = work[work["Department / Funding Bucket"].astype(str) == selected_bucket]
    return (
        work["Work Order ID"].astype(str).fillna("")
        + " - "
        + work["Work Item"].astype(str).fillna("")
    ).tolist()
...
def get_job_row(jobs_df, job_id):
    if jobs_df.empty or not job_id:
        return None
    match = jobs_df[jobs_df["Work Order ID"].astype(str) == str(job_id)]
    if match.empty:
        return None
    return match.iloc[0]
...
def get_job_context(jobs_df, job_id):
    ...
    return (
        ...
        str(row.get("Department / Funding Bucket", "")).strip(),
...
def filter_by_scope(df, selected_company=None, selected_bucket=None, selected_job_id=None):
    ...
    if selected_company and "Company Name" in work.columns:
        work = work[work["Company Name"].astype(str) == selected_company]
    if selected_bucket and "Department / Funding Bucket" in work.columns:
        work = work[work["Department / Funding Bucket"].astype(str) == selected_bucket]
    if selected_job_id and "Work Order ID" in work.columns:
        work = work[work["Work Order ID"].astype(str) == selected_job_id]
    return work
                ...
                labor_rows.append(
                    {
                        ...
                        "Department / Funding Bucket": str(row.get("Department / Funding Bucket", "")).strip(),
                        "Work Order ID": str(row.get("Work Order ID", "")).strip(),
                        "Work Item": str(row.get("Work Item", "")).strip(),
                        "Category": "Labor",
...
JOB_COLUMNS = ["Company Name", "Department / Funding Bucket", "Record Type", "Work Order ID", "Work Item", "Status", "Start Date", "End Date", "Notes"]
LABOR_COLUMNS = ["Company Name", "Department / Funding Bucket", "Work Order ID", "Work Item", "Worker Name", "Role", "Hourly Rate", "Hours Per Week", "Start Date", "End Date", "Active"]
LABOR_BUDGET_COLUMNS = ["Company Name", "Department / Funding Bucket", "Work Order ID", "Work Item", "Total Labor Amount", "Start Date", "End Date", "Spread Method", "Active", "Notes"]
CASHFLOW_COLUMNS = ["Type", "Company Name", "Department / Funding Bucket", "Work Order ID", "Work Item", "Category", "Vendor/Source", "Description", "Amount", "Date", "Spread Method", "Start Date", "End Date"]
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs([
    "Dashboard", "Companies", "Departments / Funding Buckets", "Projects / Work Orders", "Labor", "Materials and Other Costs", "6-Month Weekly Cash Flow", "Reports / Import / Export", "User Access"
])
...
with tab1:
    ...
    bucket_filter_options = ["All Departments / Funding Buckets"]
    if selected_company:
    ...
    dashboard_bucket_filter = st.selectbox("Department / Funding Bucket Filter", bucket_filter_options, key="dashboard_bucket_filter")
    selected_bucket = None if dashboard_bucket_filter == "All Departments / Funding Buckets" else dashboard_bucket_filter

    job_filter_options = ["All Projects / Work Orders"]
    if not jobs_df.empty:
        job_filter_options += job_options_list(jobs_df, selected_company, selected_bucket)
    dashboard_job_filter = st.selectbox("Project / Work Order Filter", job_filter_options, key="dashboard_job_filter")
    selected_job_id = None
    if dashboard_job_filter != "All Projects / Work Orders":
...
with tab3:
    st.subheader("Departments / Funding Buckets")
    ...
    with st.form("add_bucket_form", clear_on_submit=True):
        ...
        with c1:
            ...
            bucket_name = st.text_input("Department / Funding Bucket Name")
        with c2:
        ...
        submitted_bucket = st.form_submit_button("Add Department / Funding Bucket")
        if submitted_bucket:
    ...
    if not buckets_df.empty:
        ...
        st.info("No departments / funding buckets added yet.")

with tab4:
    st.subheader("Projects / Work Orders")
    st.caption("Use this page to connect a company and department/funding bucket to a project, work order ID, and work item. When available, use the Work Order ID from the Renovation Management System.")
    company_options = company_options_list(companies_df)
    with st.form("add_job_form", clear_on_submit=True):
        ...
        with c1:
            ...
            selected_bucket_for_job = st.selectbox("Department / Funding Bucket", bucket_options, key="job_bucket")
            job_type = st.selectbox("Record Type", JOB_TYPES)
        with c2:
            job_id = st.text_input("Work Order ID")
            job_name = st.text_input("Work Item")
        ...
        submitted_job = st.form_submit_button("Add Project / Work Order")
        if submitted_job:
            if not selected_company_for_job:
                ...
                st.error("Work Order ID and Work Item are required.")
            elif job_end < job_start:
                st.error("End Date cannot be earlier than Start Date.")
            elif job_id.strip() in jobs_df["Work Order ID"].astype(str).tolist():
                st.error("Work Order ID already exists.")
            else:
                jobs_df = pd.concat([jobs_df, pd.DataFrame([{"Company Name": selected_company_for_job, "Department / Funding Bucket": selected_bucket_for_job, "Record Type": job_type, "Work Order ID": job_id.strip(), "Work Item": job_name.strip(), "Status": job_status, "Start Date": job_start, "End Date": job_end, "Notes": job_notes.strip()}])], ignore_index=True)
...
with tab5:
    ...
    with st.form("add_quick_labor_budget_form", clear_on_submit=True):
        c1, c2, c3 = st.columns(3)
        with c1:
            selected_budget_job = st.selectbox("Project / Work Order", [""] + job_options, key="budget_job")
        ...
        if submitted_budget:
            if not selected_budget_job:
                st.error("Please select a project / work order.")
    ...
    with st.form("add_labor_form", clear_on_submit=True):
        ...
        with c1:
            ...
            selected_job = st.selectbox("Project / Work Order", [""] + job_options, key="detailed_labor_job")
...
with tab6:
    ...
    st.caption("Use this page for non-labor project/work order costs such as materials, subcontractors, permits, equipment, and other direct costs. Available funding is established on the Departments / Funding Buckets page.")
    ...
    with st.form("other_costs_form", clear_on_submit=True):
        c1, c2, c3, c4 = st.columns(4)
        with c1:
            selected_job_out = st.selectbox("Project / Work Order", [""] + job_options, key="other_costs_job")
                            ...
                            [
                                {
                                    ...
                                    "Department / Funding Bucket": bucket_name,
                                    "Work Order ID": job_id,
                                    "Work Item": job_name,
...
with tab7:
    ...
    if selected_company:
        ...
        bucket_filter_options += buckets_df["Bucket Name"].astype(str).dropna().unique().tolist() if not buckets_df.empty else []
    weekly_bucket_filter = st.selectbox("Department / Funding Bucket Filter", bucket_filter_options, key="weekly_bucket_filter")
    selected_bucket = None if weekly_bucket_filter == "All Departments / Funding Buckets" else weekly_bucket_filter

    job_filter_options = ["All Projects / Work Orders"]
    if not jobs_df.empty:
        job_filter_options += job_options_list(jobs_df, selected_company, selected_bucket)
    weekly_job_filter = st.selectbox("Project / Work Order Filter", job_filter_options, key="weekly_job_filter")
    selected_job_id = None
    if weekly_job_filter != "All Projects / Work Orders":
    ...
    if not combined_future.empty:
        ...
        weekly_detail = combined_future.groupby(["Week Start", "Company Name", "Department / Funding Bucket", "Work Order ID", "Work Item"], dropna=False)[["Cash In", "Cash Out"]].sum().reset_index().sort_values(["Week Start", "Company Name", "Department / Funding Bucket", "Work Order ID"])
...
with tab8:
    ...
    if selected_company:
        ...
        bucket_filter_options += buckets_df["Bucket Name"].astype(str).dropna().unique().tolist() if not buckets_df.empty else []
    report_bucket_filter = st.selectbox("Department / Funding Bucket Filter", bucket_filter_options, key="report_bucket_filter")
    selected_bucket = None if report_bucket_filter == "All Departments / Funding Buckets" else report_bucket_filter

    job_filter_options = ["All Projects / Work Orders"]
    if not jobs_df.empty:
        job_filter_options += job_options_list(jobs_df, selected_company, selected_bucket)
    report_job_filter = st.selectbox("Project / Work Order Filter", job_filter_options, key="report_job_filter")
    selected_job_id = None
    if report_job_filter != "All Projects / Work Orders":
    ...
    if not report_cash.empty:
        summary = report_cash.groupby(["Company Name", "Department / Funding Bucket", "Work Order ID", "Work Item", "Category"])[["Amount"]].sum().reset_index()
    ...
    export_bytes = to_excel_bytes({
        ...
        "Projects / Work Orders": jobs_df.copy(),
        "Labor": labor_df.copy(),
