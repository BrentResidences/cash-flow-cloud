...
with tab5:
    ...
    st.markdown("---")
    st.markdown("### Edit / Delete Quick Budget Forecast")
    st.caption("Choose an existing quick budget forecast to edit amounts, dates, or delete the entire forecast.")

    if not quick_budget_df.empty:
        forecast_options = unique_nonblank(quick_budget_df["Project"].astype(str).tolist())
        selected_quick_forecast = st.selectbox(
            "Choose Quick Budget Forecast to Edit",
            [""] + forecast_options,
            key="edit_quick_budget_forecast_select",
        )

        if selected_quick_forecast:
            selected_forecast_df = quick_budget_df[
                quick_budget_df["Project"].astype(str) == selected_quick_forecast
            ].copy()

            edit_display = selected_forecast_df[["Category", "Amount", "Start Date", "End Date"]].copy()
            edit_display["Amount"] = pd.to_numeric(edit_display["Amount"], errors="coerce").fillna(0.0)
            edit_display["Start Date"] = pd.to_datetime(edit_display["Start Date"], errors="coerce").dt.date
            edit_display["End Date"] = pd.to_datetime(edit_display["End Date"], errors="coerce").dt.date
            edit_display = edit_display.reset_index().rename(columns={"index": "Original Row Number"})

            edited_forecast_name = st.text_input(
                "Forecast Name",
                value=selected_quick_forecast,
                key="edit_quick_budget_forecast_name",
            )

            edited_quick_budget_df = st.data_editor(
                edit_display,
                use_container_width=True,
                num_rows="dynamic",
                key="edit_quick_budget_data_editor",
                column_config={
                    "Original Row Number": st.column_config.NumberColumn("Original Row Number", disabled=True),
                    "Category": st.column_config.SelectboxColumn("Type", options=["Labor", "Materials"], required=True),
            ...
            save_edit_col, delete_edit_col = st.columns(2)

            with save_edit_col:
                if st.button("Save Quick Budget Changes", key="save_quick_budget_changes"):
                    if not edited_forecast_name.strip():
                        st.error("Forecast Name is required.")
                    else:
                        replacement_rows = []
                        edit_error = False

                        for _, edit_row in edited_quick_budget_df.iterrows():
                            category_value = str(edit_row.get("Category", "")).strip()
                            amount_value = pd.to_numeric(pd.Series([edit_row.get("Amount", 0)]), errors="coerce").fillna(0).iloc[0]
                            start_value = pd.to_datetime(edit_row.get("Start Date"), errors="coerce")
                            end_value = pd.to_datetime(edit_row.get("End Date"), errors="coerce")

                            if float(amount_value) <= 0:
                                continue

                            if category_value not in ["Labor", "Materials"]:
                                st.error("Each edited row must have a Type of Labor or Materials.")
                                edit_error = True
                                break

                            if pd.isna(start_value) or pd.isna(end_value):
                                st.error("Each edited row with an amount must have both a Start Date and End Date.")
                                edit_error = True
                                break

                            if end_value < start_value:
                                st.error("An end date cannot be earlier than its start date.")
                                edit_error = True
                                break

                            start_date_value = start_value.date()
                            end_date_value = end_value.date()

                            replacement_rows.append({
                                "Type": "Cash Out",
                                "Company Name": "",
                                "Department": "",
                                "Project": edited_forecast_name.strip(),
                                "Category": category_value,
                                "Work Group ID": "",
                                "Work Item": "",
                                "Vendor/Source": "Quick Budget Forecast",
                                "Description": f"{edited_forecast_name.strip()} | {category_value} | {start_date_value} to {end_date_value}",
                        ...
                        if not edit_error:
                            if not replacement_rows:
                                st.error("At least one edited row must have an amount greater than zero.")
                            else:
                                cashflow_df = cashflow_df.drop(index=selected_forecast_df.index).reset_index(drop=True)
                                cashflow_df = pd.concat([cashflow_df, pd.DataFrame(replacement_rows)], ignore_index=True)
                                save_csv(cashflow_df, CASHFLOW_FILE)
                                st.success("Quick budget forecast updated.")
                                st.rerun()

            with delete_edit_col:
                confirm_delete_quick_budget = st.checkbox(
                    "Confirm delete this entire quick budget forecast",
                    key="confirm_delete_quick_budget_forecast",
                )
                if st.button("Delete Entire Quick Budget Forecast", key="delete_entire_quick_budget_forecast"):
                    if not confirm_delete_quick_budget:
                        st.warning("Please check the confirmation box before deleting.")
                    else:
                        cashflow_df = cashflow_df.drop(index=selected_forecast_df.index).reset_index(drop=True)
                        save_csv(cashflow_df, CASHFLOW_FILE)
                        st.success("Quick budget forecast deleted.")
                        st.rerun()
    else:
        st.info("No quick budget forecasts available to edit or delete.")
