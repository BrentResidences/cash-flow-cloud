...
st.set_page_config(page_title="Cash Flow App V9 Cloud", layout="wide")
...
BUCKETS_FILE = "budget_buckets"
JOBS_FILE = "projects"
LABOR_FILE = "labor"
LABOR_BUDGET_FILE = "labor_budget"
CASHFLOW_FILE = "cashflow"
...
def get_database_url():
    ...
    st.error("Database connection is missing. Add DATABASE_URL to Streamlit Cloud secrets.")
    st.stop()
...
def ensure_table(table_name, columns):
    col_defs = ", ".join([f"{qident(col)} TEXT" for col in columns])
    with get_conn() as conn:
        conn.execute(f"CREATE TABLE IF NOT EXISTS {qident(table_name)} ({col_defs});")
        ...
        for col in columns:
            if col not in existing_cols:
                conn.execute(f"ALTER TABLE {qident(table_name)} ADD COLUMN {qident(col)} TEXT;")
        conn.commit()


def load_csv(path, columns):
    table_name = TABLE_MAP.get(str(path), str(path))
    ensure_table(table_name, columns)
    select_cols = ", ".join(qident(col) for col in columns)
    with get_conn() as conn:
        rows = conn.execute(f"SELECT {select_cols} FROM {qident(table_name)};").fetchall()
...
def save_csv(df, path):
    ...
    ensure_table(table_name, columns)
    insert_cols = ", ".join(qident(col) for col in columns)
    placeholders = ", ".join(["%s"] * len(columns))
    values = [tuple(clean_db_value(row[col]) for col in columns) for _, row in df.iterrows()]
    with get_conn() as conn:
        conn.execute(f"DELETE FROM {qident(table_name)};")
        if values:
            conn.executemany(f"INSERT INTO {qident(table_name)} ({insert_cols}) VALUES ({placeholders});", values)
        conn.commit()
